...
export const Card = ({
  ...
  return (
    <>
      <AnimatePresence>
        {open && (
          <div className="fixed inset-0 h-5 overflow-auto max-h-[30rem]">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="bg-black/80 backdrop-blur-lg h-[100px] w-full fixed inset-0"
            />
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
      ...
      <motion.button
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className="rounded-3xl bg-gray-100 dark:bg-neutral-900 h-80 w-56 md:h-[40rem] md:w-96 overflow-hidden flex flex-col items-start justify-start relative z-10 max-h-[30rem] max-w-[20rem]"
      >
...
export const BlurImage = ({
  ...
  return (
    <Image
      className={cn(
        "transition duration-300 max-h-[30rem] max-w-[20rem]  !object-fill",
